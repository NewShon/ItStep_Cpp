...
int MyVector::operator[](const int id){
	if (id >=0 && id < size) return arr[id];
	return 0;
}
...
bool MyVector::operator!=(const MyVector& newVector) { return !(*this == newVector); }
bool MyVector::operator<(const MyVector& newVector) { 
	if (size < newVector.size) return 1;
	return 0;
}
bool MyVector::operator>(const MyVector& newVector) { 
	if (size > newVector.size) return 1;
